...
add_filter("plugin_action_links_$plugin", 'our_progress_settings_link' );
...
function our_progress_settings_link( $links ) { 
  $settings_link = '<a href="options-general.php?page=ourprogressmanage">Settings</a>'; 
  array_unshift( $links, $settings_link ); 
  return $links; 
}

function ourprogress_add_pages() {
    add_options_page('Our Progress', 'Our Progess', 8, 'ourprogressmanage', 'ourprogress_manage_page');
}
...
function ourprogress_manage_page() {
	...
	<table class="form-table">
		...
		<tr class="form-field form-required">
			...
			?>" size="40" aria-required="true" />
            <p>Number formating is based on the standard <a href='http://ca.php.net/manual/en/function.money-format.php' target="_blank">PHP money format</a>.</p></td>
		</tr>
        <?php }?>
		<tr class="form-field">
            ...
            <p>Which theme would you like to use for your <em>Our Progress</em> graphic?</p></td>
		</tr>
	</table>	
	<p class="submit">
...
function show_ourprogress_graphic() {
	...
	if(get_option("ourprogressformat")) 	{$format = get_option("ourprogressformat");} else {$format = "$%(#10n";}
	if(get_option("ourprogresstheme")) 		{$theme = get_option("ourprogresstheme");} else {$theme = "default-blue";}
